...
def load_data(config) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    print(f"Data splits:")
    print(
        f"  Train: {len(train_df)} samples ({train_df[config.date_col].min()} to {train_df[config.date_col].max()})"
    )
    print(
        f"  Val:   {len(val_df)} samples ({val_df[config.date_col].min()} to {val_df[config.date_col].max()})"
    )
    print(
        f"  Test:  {len(test_df)} samples ({test_df[config.date_col].min()} to {test_df[config.date_col].max()})"
    )
...
def create_dataloaders(config) -> Tuple[DataLoader, DataLoader, DataLoader]:
    ...
    # num_workers=0 is more stable for MPS
    pin_memory = config.device == "cuda"
    num_workers = 0 if config.device == "mps" else 4
