...
def train_mode(config):
    ...
    # Plot predictions
    plot_predictions(test_actuals, test_preds, save_path="test_predictions.png")
...
def eval_mode(config, checkpoint_path):
    ...
    # Plot predictions
    plot_predictions(test_actuals, test_preds, save_path="eval_predictions.png")
...
def main():
    ...
    parser.add_argument(
        "--config", type=str, default=None, help="Path to custom config file (optional)"
    )
    ...
    # Check device availability and prioritize GPU
    if torch.cuda.is_available():
        config.device = "cuda"
